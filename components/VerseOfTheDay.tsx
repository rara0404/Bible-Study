...
export function VerseOfTheDay({
      ...
      const loadLikeStatus = async () => {
        try {
          const res = await fetch(
            `/api/verse-of-day/like?user_id=${userId}&book=${encodeURIComponent(
              verse.book
          ...
          console.error(
            "Failed to load verse-of-day like status from server:",
            err
          );
        }
      };
  ...
  const toggleLike = async () => {
    ...
    if (userId) {
      try {
        console.log("[VerseOfTheDay] Toggling like:", {
          userId,
          book: verse.book,
        ...
        console.log(
          "[VerseOfTheDay] Toggle like response:",
          res.status,
          res.statusText
        );
        if (res.ok) {
        ...
        console.error(
          "[VerseOfTheDay] Failed to save verse-of-day like to server:",
          err
        );
    ...
    try {
      ...
      if (response.verses && response.verses.length > 0) {
        ...
        const convertedVerse: BibleVerse = {
          book:
            (response as any).book_name ||
            (apiVerse as any).book_name ||
            (apiVerse as any).book ||
            "Unknown",
