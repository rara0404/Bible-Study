...
import { StickyNote, AlertCircle, Heart, ChevronLeft, ChevronRight, X, CloudCog } from "lucide-react";
...
function italicizeQuotes(text: string): string {
  // Curly quotes
  let out = text.replace(/ΓÇ£([^ΓÇ¥]+)ΓÇ¥/g, '<em>$1</em>');
...
function groupVersesIntoParagraphs(verses: BibleVerse[], maxLen = 600): BibleVerse[][] {
  ...
  verses.forEach(v => {
    ...
    const endsSentence = /[.!?]["ΓÇ¥']?\s*$/.test(v.text);
...
export function BibleReader(props: BibleReaderProps) {
  ...
  const saveNote = async () => {
    ...
    const newNote: Note = { book, chapter, verse: selectedVerse, note: trimmed, content: trimmed };
  ...
  // Keyboard: ΓåÉ previous, ΓåÆ next
  useEffect(() => {
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="space-y-5 text-[15px] sm:text-[16px] leading-7 text-gray-900 dark:text-gray-100">
                  ...
                  <div key={idx} className="relative selection:bg-blue-100/70 dark:selection:bg-blue-900/40" suppressHydrationWarning>
                    {para.map(v => {
                              ...
                              >
                                ...
                                {getVerseNote(v.verse) && (
                                  <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-800">
                                    ...
                                    <div className="text-sm text-gray-800 dark:text-gray-200 mb-3 whitespace-pre-wrap">{getVerseNote(v.verse)?.content}</div>
                                    <button
                                      type="button"
